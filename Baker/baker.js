(function ($) {
	...
	var volume, player, score, sumScore, stop, ticker;
	...
	if (canUseLocalStorage) {
		var playSound = localStorage.getItem("game.playSound");
	...
	var assetLoader = (function() {
		...
		this.checkAudio  = {};                               // Variable for checking audio loading.
	...
	function Animation(spritesheet, frameSpeed, startFrame, endFrame) {
		...
		var animationSequence = [];  // Array holding the order of the animation the current frame to draw keep track of frame rate.
		var currentFrame = 0;
		var counter = 0;
	...
	*/
	...
	function updateEnvironment() {
		...
		for (var i = 0; i < environment.length; i++) {
			...
			// Sobiranie plushek:
			var angle;
			if (player.minDist(environment[i]) <= player.width - platformWidth/2) {
				environment.splice(i, 1);
				i--;

				assetLoader.sounds.bite.currentTime = 0;
				assetLoader.sounds.bite.play();

				sumScore = sumScore + Math.round(score / 100);
			}
		}
	...
	function animate() {
		if (!stop) {
			...
			// Draw the score:
			ctx.fillText('Счёт: ' + sumScore, canvas.width - 180, 30);
